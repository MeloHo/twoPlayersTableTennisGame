...
int main(int argc, char *argv[])
{
  ...
  SOCKET serverSd = server.StartServer();
  ...
  while(true)
  {
      ...
      if (bytecount = send(server.client1Sd, (char*)pkt1, siz1, 0) == -1){
      ...
      if (bytecount = send(server.client2Sd, (char*)pkt2, siz2, 0) == -1){
  ...
  closesocket(server.client1Sd);
  closesocket(server.client2Sd);
